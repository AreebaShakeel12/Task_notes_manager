...
app.secret_key = os.environ.get('SECRET_KEY', 'a_long_and_random_string_for_dev_safety_CHANGEME_IN_PROD')

# 2. UPLOAD_FOLDER: For file uploads (use a relative path)
# Default to 'static/uploads' if UPLOAD_FOLDER env var is not set.
app.config['UPLOAD_FOLDER'] = os.environ.get('UPLOAD_FOLDER', 'static/uploads')

# 3. Mail Configuration: All details from environment variables
app.config.update(
    MAIL_SERVER=os.environ.get('MAIL_SERVER', 'smtp.gmail.com'),
    MAIL_PORT=int(os.environ.get('MAIL_PORT', 465)),  # Convert to int
    MAIL_USE_SSL=os.environ.get('MAIL_USE_SSL', 'True').lower() in ('true', '1', 't'),  # Convert to boolean
    MAIL_USERNAME=os.environ.get('MAIL_USERNAME'),  # MUST be set in production
    MAIL_PASSWORD=os.environ.get('MAIL_PASSWORD')   # MUST be set in production
)
mail = Mail(app)
...
# In production, Render/Railway WILL set DATABASE_URL.
...
class Task(db.Model):
    ...
    # Add description if you use it in your forms (uncomment below if needed in DB):
